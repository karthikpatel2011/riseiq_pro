import { timeAgo } from "../../hooks/useFeed";

function Avatar({ authorName, authorPhotoURL }) {
  const initial = authorName ? authorName.charAt(0).toUpperCase() : "?";
  if (authorPhotoURL) {
    return (
      <img
        className="dash-card-avatar"
        src={authorPhotoURL}
        alt={authorName}
      />
    );
  }
  return <div className="dash-card-avatar dash-card-avatar--init">{initial}</div>;
}

function DoubtCard({ item, onTap }) {
  const {
    title,
    body,
    subject,
    branch,
    authorName,
    authorPhotoURL,
    answerCount,
    createdAt,
  } = item;

  function handleKeyDown(e) {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      onTap(item);
    }
  }

  function stop(e) {
    e.stopPropagation();
  }

  return (
    <article
      className="dash-card dash-card--doubt"
      role="button"
      tabIndex={0}
      onClick={() => onTap(item)}
      onKeyDown={handleKeyDown}
    >
      <div className="dash-card-tags">
        <span className="dash-card-tag dash-card-tag--doubt">Doubt</span>
        <span className="dash-card-subject">
          {subject}
          {branch ? ` · ${branch}` : ""}
        </span>
      </div>

      <h3 className="dash-card-title">{title}</h3>

      {body && <p className="dash-card-body">{body}</p>}

      <div className="dash-card-footer">
        <div className="dash-card-user">
          <Avatar authorName={authorName} authorPhotoURL={authorPhotoURL} />
          <span className="dash-card-username">{authorName}</span>
          <span className="dash-card-dot">·</span>
          <span className="dash-card-time">{timeAgo(createdAt)}</span>
        </div>

        <span className="dash-card-answers">
          {answerCount || 0} answer{answerCount === 1 ? "" : "s"}
        </span>
      </div>

      <div className="dash-card-actions">
        <button className="dash-card-action" onClick={stop}>
          ▲ Upvote
        </button>
        <button className="dash-card-action" onClick={stop}>
          💬 Comment
        </button>
        <button className="dash-card-action" onClick={stop}>
          🔖 Save
        </button>
      </div>
    </article>
  );
}

export default DoubtCard;
